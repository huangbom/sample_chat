...
import com.hzx.luoyechat.applib.controller.HXSDKHelper.HXSyncListener;
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				if (Constant.NEW_FRIENDS_USERNAME.equals(username)) {
					...
					intent.putExtra("userId",Constant.CHAT_KEFU);
...
//		});
		listView.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> adapterView, View view, int i, long l) {
				if (i > 2) {
					toBeProcessUser = adapter.getItem(i);
					new android.support.v7.app.AlertDialog.Builder(getActivity())
							.setTitle("提示")
							.setMessage("确定要删除 "+toBeProcessUser.getNick()+" 吗？")
	...
	private void getContactList() {
		...
		while (iterator.hasNext()) {
			...
			if (!entry.getKey().equals(Constant.NEW_FRIENDS_USERNAME)
					...
					&& !entry.getKey().equals(Constant.CHAT_KEFU)
		...
		if(users.get(Constant.CHAT_KEFU)!=null){
			contactList.add(0, users.get(Constant.CHAT_KEFU));
		}
		// 加入"群聊"和"聊天室"
